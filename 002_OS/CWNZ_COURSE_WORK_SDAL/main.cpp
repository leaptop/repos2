#include <windows.h>//СНИФФЕР КУРСАЧ ДОРАБОТАТЬ
#include <iostream>
#include <string>
#include <fstream>
...
//mouseHookProc - хранитель для определенного приложением  имени функции. Короче просто имя функции, которое я задал.ther
//uMsg - у нас это nCode? - само сообщение
LRESULT CALLBACK mouseHookProc(int nCode, WPARAM wParam,
                               LPARAM lParam) {//mouseHookProc CALLBACK функция, которая обрабатывает сообщения, отправленные окну.
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsance, LPSTR lpCmdLine,
                   int nShowCdm) { //короче так запускается виндовская прога
