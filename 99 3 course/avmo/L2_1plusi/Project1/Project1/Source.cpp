...
long long fact(long long n)//считаем факториал
{
	if (n <= 1)
		return 1;
...
long long combinationNum(long long a, long long b)//считаем число комбинаций
{
	if (a == b)
		return 1;
...
bool NextSet(int* a, int n, int m)//не пон€тно
{
	int k = m;
	for (int i = k - 1; i >= 0; --i)
		if (a[i] < n - k + i + 1)
...
void read(Fraction** arr, int n, int m)//принимаю матрицу с клавиатуры
{
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
...
void print(Fraction** arr, int n, int m)//вывожу матрицу на экран
{
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n - 1; j++)
		{
			cout << setw(7) << arr[i][j] << " ";//«ƒ≈—№ ѕ–ќ—“ќ Ў»–»Ќј ќ„≈¬»ƒЌќ ‘ќ–ћј“»–”≈“—я
		}
...
void swap(Fraction** arr, int n, int from, int to)//мен€ю местами дроби... n штук переношу из одной строки в другую 
{
	Fraction c;
	for (int i = 0; i < n; i++)
	{
...
void copy(Fraction** from, Fraction** to, int n, int m)//копирую дроби в **to
{
	for (int i = 0; i < m; i++)
	{
		for (int j = 0; j < n; j++)
...
void JordanGauss(Fraction** arr, int n, int m)// главна€ функци€
{
...
{
	int n, m;
	cin >> n >> m;//прин€л две переменные дл€ размера матрицы

	Fraction** arr = new Fraction * [m];//создал массив указателей на указатели типа Fraction

	for (int i = 0; i < m; i++)
	{
		arr[i] = new Fraction[n];//выделил пам€ть дл€ каждого указател€ на указатели(особенность создани€ двумерных массивов)
	}

	read(arr, n, m);//ввЄл матрицу с клавиатуры
	print(arr, n, m);//напечатал эту матрицу дл€ проверки
	JordanGauss(arr, n, m);
