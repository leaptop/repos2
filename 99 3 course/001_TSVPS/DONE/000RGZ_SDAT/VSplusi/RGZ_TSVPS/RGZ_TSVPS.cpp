// RGZ_TSVPS.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
double blockMultiplicationTimer(int order, int blockSize) {
	...
	double* matrix1 = static_cast<double*>(malloc(size * sizeof(double)));//double* matrix1 = malloc(size * sizeof(double));
	double* matrix2 = static_cast<double*>(malloc(size * sizeof(double)));
	// Матрицу с результатом умножения надо инициализировать нулями, т.е. при помощи calloc, а не malloc
	double* resultMatrix = static_cast<double*>(calloc(size, sizeof(double))); //double* resultMatrix = calloc(size, sizeof(double));
...
void part2() {
	...
	for (int blockSize = 1; blockSize <= order; blockSize *= 2) {
		printf("blockSize: %d - %f sec\n", blockSize, blockMultiplicationTimer(order, blockSize));
	}
}
