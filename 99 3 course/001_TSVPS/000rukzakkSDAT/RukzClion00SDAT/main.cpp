...
vector<int> get_set_products(vector<int> prev, vector<int> m, int W, int* weighPacked) {
    ...
    //while(weight>=0){
        ...
        weight -= temp;
        *weighPacked = W - weight;
    }
    return indexes;
}

double knapsackSolver(vector<int> m, vector<double> c, int W, int n, int *wei) {//m - mass,  c -cost
    ...
    set = get_set_products(prev, m, W, wei);
    return f[f.size() - 1];
}

int main() {
    system("chcp 65001");
    int W, N, weightPacked = 0;
    int*wei = &weightPacked;
    char ch;
    int num;
    double num_1;
