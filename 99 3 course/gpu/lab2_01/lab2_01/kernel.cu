...
#include <time.h> 
#include <iostream>
#include <fstream>///что с этим делать? (Код из лекции 2)
...
void launchKernel(int N, int threadsPerBlock, float* time_gl, int* N_gl) {
	int blocksPerGrid = (N + threadsPerBlock - 1) / threadsPerBlock;//здесь сама функция с защитой от дурака получается
	//и чтобы сделать переполнение нитей(задать больше 1024), надо увеличить и threadsPerBlock и N... Хотя
	...
	float elapsedTime;  cudaEvent_t    start, stop; // встроенный тип данных – структура, для     // фиксации контрольных точек    
...
void testFunction() {
	...
	for (int i_thr = threadsPerblock_local; i_thr <= 2049; i_thr *= 2)//графиков д.б. столько, сколько у меня конфигураций нитей - 10
	{
		if (out.is_open())
		{
			out << "\n\n" << std::endl;
