/*сначала запустить service1.cpp так: 
cl service1.cpp Advapi32.lib    
service1 setup
жму значок виндовс, набираю "службы", ищу имя службы(здесь задано wserv_test), слева нажимаю "запустить службу". Дольше пока не прошёл. Буду пытаться свой код вставить.
С родным кодом ничё не происходило, но я создал папку C:\Test и там появился таки файл лога, и туда записывалось время каждые 5 секунд.
Чтобы посмотреть есть ли всё еще служба, жму правой кнопкой и обновить. 


из лекций кавычки могут не так браться*/
