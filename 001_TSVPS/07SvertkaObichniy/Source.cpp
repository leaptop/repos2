...
using namespace std;
int count_ = 0, count_dp = 0, count_pb_1 = 0, count_pb_2 = 0, count_pb_3 = 0, count_b_1 = 0, count_b_2 = 0;
int size_;
const int r = 4;
int p_1, p_2;
...
void half_quick_transform(vector <int> source, complex <double>* result) {
	int i = 0;
	complex <double>** a1;
	a1 = new complex <double> * [size_];
	for (int i = 0; i < size_; i++) {
		a1[i] = new complex <double>[size_];
	}
	for (int j_2 = 0; j_2 < p_2; j_2++) {
...
void svertka_PBPF(vector <int> a, vector <int> b, complex <double>* result) {
	...
	complex <double> res[sizez];
	complex <double> a_pbpf[sizez];
	complex <double> b_pbpf[sizez];
...
void create_A_0(complex <double>* array, complex <double>* result) {
	...
	bitset <r> set_bits;
	for (int loop = 0; loop < 2 * size_; loop++) {
...
void FFT(complex <double>* array, complex <double>* a_0) {
	...
	complex <double> rotate_multiplier = exp(complex <double>(0, (-2 * M_PI) / (2 * size_)));
	stack < complex <double> > W;
	int step = 0;
	for (step = 2 * size_; step != 1; step /= 2) {
		W.push(rotate_multiplier);
		rotate_multiplier *= rotate_multiplier;
	}
	for (step = 2; step <= 2 * size_; step *= 2) {
		rotate_multiplier = W.top();
		W.pop();
		for (int i = 0; i < 2 * size_; i += step) {
...
void back_FFT(complex <double>* array) {
	complex <double>* result = new complex <double>[2 * size_];
	for (int i = 0; i < 2 * size_; i++) {
...
void svertka_BPF(complex <double>* a, complex <double>* b, complex <double>* result) {
	complex <double>* a_bpf = new complex <double>[2 * size_];
	complex <double>* b_bpf = new complex <double>[2 * size_];
	FFT(a, a_bpf);
	FFT(b, b_bpf);
	for (int i = 0; i < 2 * size_; i++) {
...
int main() {
	...
	scanf("%d", &size_);
	vector <int> arr_1(2 * size_);
	vector <int> arr_2(2 * size_);
	complex <double> result[2 * size_ - 1];
	printf("\n\nArray A: ");
	for (int i = 0; i < size_; i++) {
		arr_1[i] = i + 1;
		printf("%d ", arr_1[i]);
	...
	scanf("%d", &size_);
	vector <int> arr_1_PBPF(2 * size_);
	vector <int> arr_2_PBPF(2 * size_);
	complex <double> result_PBPF[2 * size_ - 1];
	printf("\n\nArray A: ");
	for (int i = 0; i < size_; i++) {
		arr_1_PBPF[i] = i + 1;
