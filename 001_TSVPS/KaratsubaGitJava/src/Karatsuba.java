...
public class Karatsuba {
    ...
    public static String[] strCopy(long index, String string) {//возвращает массив из двух строк, разделённых по индексу
        String	first = "",
                last = "";
        long actualIndex = string.length() - index;
        for (int i = 0; i<actualIndex; i++) {
            first+=string.charAt(i);//конкатенирую к пустой строке чары входной строки от 0 до (string.length() - index)
        }//т.е. first - старшие разряды
        for (int i = (int)actualIndex; i<string.length(); i++) {//конкатенирую к пустой строке чары входной
            last+=string.charAt(i);//строки от (string.length() - index) до (string.length())
        }//т.е. last - младшие разряды входной строки
        return new String[] {first, last};//возвращает двухэлементный(две строки) массив разделённой строки
        //Например если дано 12345 в виде строки и 2 - индекс, то разделит строку на строковый массив ["123", "45"]
    }//таким образом 123 м.б. записано как 123 * 10^m, с m = 2 как индексом
    ...
    public static void main(String[] args) {
        ...
        System.out.println(karatsuba(21, 49));
        System.out.println(karatsuba(2154, 5324));
    }
