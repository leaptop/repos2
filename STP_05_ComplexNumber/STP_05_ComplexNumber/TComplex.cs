...
namespace STP_05_ComplexNumber
{
    public class TComplex : ICloneable
    {
        ...
        {
            double module = Math.Pow(this.module(), 1d / n);//типа вычисляю таким образом корень модуля//получил корень энной степени из модуля
            double phase = (angleRadians() + 2 * Math.PI * i) / n;
            return new TComplex(module * Math.Cos(phase), module * Math.Sin(phase));
        }
