...
namespace STP_13_ParameterizedSet
{
    ...
    public class TFrac : ICloneable, IEnumerable<int>, InterfaceForNumbers<TFrac>, IComparableNumber<TFrac>, IComparable<TFrac>
    {
        static void MainFrac(string[] args)
        {
            TFrac tf;
            try
            {
        ...
        public TFrac rev(TFrac a)//нахождение обратной дроби = rev
        {
            if (a.numerator != 0)
        ...
        public int CompareTo(TFrac other)
        {
            if (this.isMoreThanParameter_d(other))
            {
                return 1;
            }
            else if (this.thisEqualToParameter_d(other))
            {
                return 0;
            }
            else return -1;
