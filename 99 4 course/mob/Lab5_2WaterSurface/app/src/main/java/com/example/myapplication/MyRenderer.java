...
//сделать капли по всему экрану, а не только в левом нижнем углу. Сделал.
...
class MyRenderer implements GLSurfaceView.Renderer {
    ...
    void display() {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N - 1; j++) {
                a[N * i * a3 * a2 + j * a3 * a2 + 0] = 1.0f * j / N;
                a[N * i * a3 * a2 + j * a3 * a2 + 1] = 1.0f * i / N;
                a[N * i * a3 * a2 + j * a3 * a2 + 2] = 1.0f * (p[i][j]).z;
                a[N * i * a3 * a2 + j * a3 * a2 + 3] = 1.0f * (j + 1) / N;
                a[N * i * a3 * a2 + j * a3 * a2 + 4] = 1.0f * (i) / N;
                a[N * i * a3 * a2 + j * a3 * a2 + 5] = 1.0f * (p[i][j + 1]).z;
                offs += 6;
            }
        }
        for (int i = 0; i < N - 1; i++) {
    ...
    void Push1() {
        ...
        int a2 = 1;
        int x0 = (int) (Math.random() * N / a2 + 1);
        int y0 = (int) (Math.random() * N / a2 + 1);
        for (int y = y0 - 5; y < y0 + 5; y++) {
            if ((y < 1) || (y >= N - 1)) continue;
            for (int x = x0 - 5; x < x0 + 5; x++) {
                if ((x < 1) || (x >= N - 1)) continue;
                p[x][y].z = 10.0f / N - (float) (Math.sqrt(sqr(y - y0) + sqr(x -
                        x0)) * 1.0 / N);
            }
        }
    }
    ...
    void MyTimer() {
        ...
        for (int y = 1; y < N - 1; ++y) {
            for (int x = 1; x < N - 1; ++x) {
                //p[x][y].z=(p[x][y].z>0.1f)?0.1f:p[x][y].z;
                //p[x][y].z=(p[x][y].z<-0.1f)?-0.1f:p[x][y].z;
                P p0 = p[x][y];
                for (int i = 0; i < 4; ++i) {
