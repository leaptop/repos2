...
namespace STP_10_V2_ADT_TMemoryNumbersInsertedLikeFiles
{
    ...
    public class TFrac : ICloneable, IEnumerable<int>, InterfaceForNumbers<TFrac>
    {
        static void MainTFrac(string[] args)
        {
            TFrac tf;
            try
            {
        ...
        TFrac InterfaceForNumbers<TFrac>.add(TFrac a, TFrac b)
        {
            TFrac aa = (TFrac)a.Clone();//Клонирование нужно, чтобы сохранить оригинальные дроби в исходном виде
            TFrac bb = (TFrac)b.Clone();
            transformToOneDenominator(ref aa, ref bb);
        ...
        public TFrac add(TFrac e)
        {
            TFrac aa = (TFrac)e.Clone();//Клонирование нужно, чтобы сохранить оригинальные дроби в исходном виде
            TFrac bb = (TFrac)this.Clone();
            transformToOneDenominator(ref aa, ref bb);
            return new TFrac(aa.numerator + bb.numerator, aa.denominator);
        }
    }
