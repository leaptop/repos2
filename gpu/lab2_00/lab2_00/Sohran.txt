...
in an undefined behavior.
	*/
	...
	//double tm = double(t2.QuadPart - t1.QuadPart) / f.QuadPart;// так можно уже в секундах вывести
	//std::cout << "\nFirstTime \n" << tm;
	...
	sum << <blocksPerGrid, threadsPerBlock >> > (dev_a, dev_b, N);//http://ecee.colorado.edu/~siewerts/extra/code/example_code_archive/a490dmis_code/CUDA/cuda_work/samples/0_Simple/vectorAdd/vectorAdd.cu
	cudaDeviceSynchronize();
	QueryPerformanceCounter(&t4);
	double tm = double(t4.QuadPart - t3.QuadPart);// / f.QuadPart;
	std::cout << "\nthreadsPerBlock: " << threadsPerBlock << ", time: " << tm << "\n";
	...
	cudaDeviceSynchronize();
	QueryPerformanceCounter(&t4);
	tm = double(t4.QuadPart - t3.QuadPart);// / f.QuadPart;
	std::cout << "\nthreadsPerBlock: " << threadsPerBlock << ", time: " << tm << "\n";
	...
	tm = double(t6.QuadPart - t5.QuadPart);// / f.QuadPart;
	std::cout <<" time: " << tm << "\n";
